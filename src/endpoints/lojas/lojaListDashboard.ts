...
export class LojaListDashboard extends OpenAPIRoute {
  public schema = {
    tags: ["Lojas"],
    summary: "Listar lojas ativas para dashboard (status <> 0)",
    operationId: "loja-list-dashboard",
    request: {
      query: z.object({
    ...
    responses: {
      "200": {
        description: "Lista de lojas ativas (status <> 0)",
        ...contentJson({
          success: Boolean,
  ...
  public async handle(...[c]: HandleArgs) {
    ...
    // SEMPRE filtrar por status <> 0 para o dashboard
    let whereClause = "WHERE status <> 0";
    const params: any[] = [];
    if (search && search.trim() !== "") {
      whereClause = "WHERE status <> 0 AND (banco LIKE ? OR loja LIKE ?)";
